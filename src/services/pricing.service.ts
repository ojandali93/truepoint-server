...
import { findCachedPrices, upsertPrices } from '../repositories/card.repository';
...
const normalizeCardMarketPrices = (
  ...
  if (data.prices.cardmarket) {
    ...
    if (cm.graded?.psa?.psa10)
      prices.push({ cardId, source: 'cardmarket', variant: 'normal', grade: 'PSA 10', lowPrice: null, midPrice: null, highPrice: null, marketPrice: cm.graded.psa.psa10, fetchedAt: now });
    if (cm.graded?.psa?.psa9)
      prices.push({ cardId, source: 'cardmarket', variant: 'normal', grade: 'PSA 9', lowPrice: null, midPrice: null, highPrice: null, marketPrice: cm.graded.psa.psa9, fetchedAt: now });
    if (cm.graded?.cgc?.cgc10)
      prices.push({ cardId, source: 'cardmarket', variant: 'normal', grade: 'CGC 10', lowPrice: null, midPrice: null, highPrice: null, marketPrice: cm.graded.cgc.cgc10, fetchedAt: now });
  }
  ...
  if (data.prices.tcg_player?.market_price) {
    prices.push({
      cardId, source: 'tcgplayer', variant: 'normal', grade: null,
      lowPrice: null,
...
export const getRawCardPrices = async (cardId: string): Promise<NormalizedPrice[]> => {
  const cacheKey = `prices:raw:${cardId}`;
...
export const getCardMarketPrices = async (
  ...
  if (!process.env.RAPIDAPI_KEY) {
    console.warn('[PricingService] RAPIDAPI_KEY not set — skipping CardMarket prices');
    return [];
  }
  ...
  try {
    ...
    if (cardMarketId) {
      ...
      const card = await pokemonTcgClient.getCardById(cardId);
      const searchResults = await cardMarketClient.searchCard(card.name, card.set.id);
    ...
    console.error('[PricingService] CardMarket fetch failed:', (err as any)?.message);
    return [];
  }
};
...
export const getJustTcgPrices = async (
  ...
  if (!process.env.JUSTTCG_API_KEY) {
    console.warn('[PricingService] JUSTTCG_API_KEY not set — skipping JustTCG prices');
    return [];
  }
  ...
  try {
    ...
    console.error('[PricingService] JustTCG fetch failed:', (err as any)?.message);
    return [];
  }
};
