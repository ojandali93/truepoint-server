...
const fetchTCGPlayerPrices = async (cardId: string): Promise<NormalizedPrice[]> => {
  try {
    const card = await pokemonTcgClient.getCardById(cardId);
    if (!card?.tcgplayer?.prices) return [];

    const now = new Date().toISOString();
    return Object.entries(card.tcgplayer.prices).map(([variant, prices]: [string, any]) => ({
      cardId,
    ...
    console.error(`[PricingService] TCGPlayer fetch failed for ${cardId}:`, err?.message);
    return [];
  }
};
...
const fetchGradedPrices = async (
  ...
  try {
    ...
    if (data.prices?.cardmarket) {
      ...
      prices.push({
        cardId, source: 'cardmarket', variant: 'normal', grade: null,
      ...
      for (const company of gradingCompanies) {
        const companyGrades = (graded as Partial<Record<(typeof gradingCompanies)[number], Record<string, number>>>)[company];
        ...
        for (const [grade, price] of Object.entries(companyGrades)) {
          ...
          prices.push({
            cardId, source: 'cardmarket', variant: 'normal',
            grade: gradeLabel,
            lowPrice: null, midPrice: null, highPrice: null,
            marketPrice: price,
            fetchedAt: now,
          });
        }
    ...
    if (data.prices?.ebay?.graded) {
      for (const [company, grades] of Object.entries(data.prices.ebay.graded as Record<string, any>)) {
        for (const [grade, gradeData] of Object.entries(grades as Record<string, any>)) {
          if (!gradeData?.median_price) continue;
          prices.push({
            cardId, source: 'ebay', variant: 'normal',
    ...
    console.error(`[PricingService] CardMarket graded fetch failed for ${cardId}:`, err?.message);
    return [];
  }
};
...
export const getAllPricesForCard = async (
  ...
  const [tcgPrices, gradedPrices] = await Promise.allSettled([
    ...
    setId ? fetchGradedPrices(cardId, cardName, setId) : Promise.resolve([]),
  ]);
  ...
  if (allPrices.length > 0) {
    await upsertPrices(allPrices, allPrices.some((p) => p.grade) ? TTL_GRADED_MS : TTL_RAW_MS).catch((err) =>
      console.error(`[PricingService] Failed to store prices for ${cardId}:`, err?.message)
    );
...
export const resolveMarketValue = async (
  ...
  if (gradingCompany && grade) {
    ...
    const graded = cached.find(
      (p) => p.grade && p.grade.toLowerCase() === gradeLabel.toLowerCase() && p.marketPrice
    );
    ...
    if (normalPrice?.marketPrice) return { price: normalPrice.marketPrice, source: 'tcgplayer' };
    ...
    // CardMarket fallback
    const cm = cached.find((p) => p.source === 'cardmarket' && !p.grade && p.marketPrice);
  ...
  if (tcgRaw?.marketPrice) return { price: tcgRaw.marketPrice, source: 'tcgplayer' };
