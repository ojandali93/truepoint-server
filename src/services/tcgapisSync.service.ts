...
export const syncSetGroupIds = async (): Promise<{
  ...
  while (true) {
    ...
    if (allExpansions.length >= data.total || (data.data?.length ?? 0) < 100)
      break;
    offset += 100;
  }
...
export const syncSetCards = async (
  setId: string,
): Promise<{
  cards: number;
  variants: number;
  ...
  while (true) {
    ...
    if (allApiCards.length >= data.total || (data.data?.length ?? 0) < 100)
      break;
    offset += 100;
  }
  ...
  for (const apiCard of allApiCards) {
    ...
    try {
      ...
      for (const variantName of variantKeys) {
        ...
        // Upsert card_variant
        await supabaseAdmin.from("card_variants").upsert(
          {
            card_id: ourCard.id,
            set_id: setId,
      ...
      await logError({
        source: "tcgapis-price-fetch", // ← change per controller
      ...
      console.error(
        `[TCGAPIs] Price fetch failed for ${apiCard.productId}:`,
        err?.message,
      );
      skipped++;
    }
...
export const refreshPricesForSet = async (
  setId: string,
): Promise<{ prices: number }> => {
  ...
  for (const card of cards ?? []) {
    ...
    try {
      ...
      for (const [variantName, priceData] of Object.entries(pricesObj)) {
        if (
          !priceData ||
          (priceData.marketPrice == null && priceData.lowPrice == null)
        )
          continue;
        const variantDef = resolveVariant(variantName);
...
export const refreshAllPrices = async (): Promise<{ totalPrices: number }> => {
  ...
  for (const set of sets ?? []) {
    try {
      ...
      await logError({
        source: "tcgapis-refresh-prices-for-set", // ← change per controller
        message: err?.message ?? "Unknown error",
        error: err,
        userId: null,
        requestPath: "",
      ...
      console.error(
        `[TCGAPIs] Price refresh failed for ${set.name}:`,
        err?.message,
      );
    }
  }
...
export const getCardSalesHistory = async (
  ...
  return tcgapisGet<any>(`/api/v2/sales-history/${productId}/full`, params);
};
