...
export const getPublicProfile = async (id: string) => {
  const profile = await UserRepository.findProfileById(id);
  if (!profile) throw { status: 404, message: 'Profile not found' };
  const { show_market_values, is_pro_member, currency, ...publicFields } = profile;
  return publicFields;
};
...
export const createProfile = async (
  ...
  if (existing) throw { status: 409, message: 'Profile already exists for this user' };
...
export const updateProfile = async (
  ...
  try {
    ...
    if (err.code === '23505') throw { status: 409, message: 'Username is already taken' };
    throw err;
  }
};
...
export const getNotificationSettings = async (
  ...
  if (!settings) throw { status: 404, message: 'Notification settings not found' };
  return settings;
};
...
export const createNotificationSettings = async (
  ...
  if (existing) throw { status: 409, message: 'Notification settings already exist' };
...
export const logActivity = async (
  userId: string,
  payload: { event_name: string; metadata?: Record<string, unknown>; platform?: string }
): Promise<void> => {
...
export const adminCreateStandardUser = async (
  ...
  const profile = await UserRepository.createProfile(authUser.id, { username, full_name: fullName });
