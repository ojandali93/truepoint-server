...
export const getAllSets = async (): Promise<PokemonSet[]> => {
  ...
  // Fallback: live API (only hits if DB is empty)
...
export const getSetById = async (setId: string): Promise<PokemonSet> => {
  ...
  if (dbSet) {
    return {
      id: dbSet.id, name: dbSet.name, series: dbSet.series,
      printedTotal: dbSet.printed_total, total: dbSet.total,
...
export const searchCards = async (
  ...
  if (!params.q && !params.setId && !params.rarity && !params.supertype && !params.type) {
...
export const syncSets = async (): Promise<{ synced: number; duration: number }> => {
