...
const parseGradeKey = (
  key: string,
): { company: string; grade: string } | null => {
...
const syncExpansionPrices = async (
  ...
  console.log(
    `[CMPriceSync] Fetching cards for expansion ${expansionId} (${expansionName})...`,
  );
  ...
  console.log(
    `[CMPriceSync] Sample: #${s.card_number} ${s.name}, tcg_player=$${s.prices?.tcg_player?.market_price}`,
  );
  ...
  for (const rawCard of cmCards) {
    ...
    if (!cardId) {
      if (result.noMatch < 3) {
        console.log(
          `[CMPriceSync] No DB match for CM #${cmCard.card_number} (normalized: ${cardNum})`,
        );
      }
      result.noMatch++;
      continue;
    ...
    // TCGPlayer prices skipped — pokemontcg.io provides accurate USD prices.
    // CardMarket's tcg_player field is EUR-converted and stale.
    ...
    if (prices?.cardmarket) {
      ...
      if (
        prices.cardmarket.graded &&
        !Array.isArray(prices.cardmarket.graded)
      ) {
        for (const [company, grades] of Object.entries(
          prices.cardmarket.graded as Record<string, any>,
        )) {
          console.log(company, grades);
          if (!grades || typeof grades !== "object") continue;
          for (const [gradeKey, price] of Object.entries(
    ...
    if (prices?.ebay?.graded && !Array.isArray(prices.ebay.graded)) {
      for (const [company, grades] of Object.entries(
        prices.ebay.graded as Record<string, any>,
      )) {
        if (!grades || typeof grades !== "object") continue;
    ...
    if (hasPrice) {
      ...
      if (result.synced <= 2) {
        console.log(
          `[CMPriceSync] ✓ #${cmCard.card_number} ${cmCard.name}: $${prices?.tcg_player?.market_price}`,
        );
      }
    } else {
      result.noPrice++;
  ...
  console.log(
    `[CMPriceSync] ${result.synced} priced, ${result.noMatch} no DB match, ${result.noPrice} no price`,
  );
...
export const syncAllPricesFromCardMarket = async (): Promise<void> => {
  ...
  console.log(
    `[CMPriceSync] Done — ${totalSynced} cards priced across ${setsProcessed} sets. ${setsSkipped} skipped.`,
  );
};
...
export const syncSetPricesFromCardMarket = async (
  ...
  const match = expansions.find(
    (e: any) => e && matchExpansionToSet(e, ourSets) === setId,
  );
  ...
  console.log(
    `[CMPriceSync] Matched: ${(match as any).name} (id: ${(match as any).id})`,
  );
  const result = await syncExpansionPrices(
    (match as any).id,
    (match as any).name ?? setId,
    setId,
  );
