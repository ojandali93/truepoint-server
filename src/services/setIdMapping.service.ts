...
export const discoverTCGdexIds = async (): Promise<{
  ...
  const tcgdexByName = new Map(
    tcgdexSets.map((s) => [normalizeName(s.name), s]),
  );
  ...
  for (const set of ourSets) {
    ...
    const directMatch =
      ...
      tcgdexById.get(
        set.id
          ...
          .replace(
            /([a-z])(\d)([a-z]|pt|$)/i,
            (_: any, p: any, d: any, s: any) => `${p}0${d}${s}`,
          )
          .toLowerCase(),
      );
    ...
    if (directMatch) {
      await supabaseAdmin
        ...
        .eq("id", set.id);
      console.log(
        `[SetIdMapping] ✓ ${set.name}: ${set.id} → ${directMatch.id} (ID match)`,
      );
      matched++;
      continue;
    }
    ...
    if (nameMatch) {
      await supabaseAdmin
        ...
        .eq("id", set.id);
      console.log(
        `[SetIdMapping] ✓ ${set.name}: ${set.id} → ${nameMatch.id} (name match)`,
      );
      matched++;
      continue;
    }
    ...
    const partialMatch = Array.from(tcgdexByName.entries()).find(
      ([tcgName]) => {
        return (
          tcgName.includes(normalizedName.slice(0, 8)) ||
    ...
    if (partialMatch) {
      ...
      await supabaseAdmin
        ...
        .eq("id", set.id);
      console.log(
        `[SetIdMapping] ~ ${set.name}: ${set.id} → ${tcgSet.id} (partial match)`,
      );
      matched++;
      continue;
    }
  ...
  if (unmatched.length > 0) {
    console.warn(
      `[SetIdMapping] ${unmatched.length} sets could not be matched to TCGdex:`,
    );
...
export const getTCGdexIdForSet = async (
  ourSetId: string,
): Promise<string | null> => {
